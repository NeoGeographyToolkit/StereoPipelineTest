...
bundle_adjust --threads 1 --max-pairwise-matches 2000 run/run-l.tif run/run-p-1000[0-4].tif run/run-l.json run/run-p-1000[0-4].json -o run/run --output-cnet-type isis-cnet --num-iterations 5 --camera-position-uncertainty run/camera_uncertainty.txt

# Solve for jitter. Use the isis cnet format, anchor points, and a DEM
jitter_solve --num-iterations 5 --rotation-weight 0.1 --threads 1 run/run-l.tif run/run-p-images.txt run/run-l.json run/run-p-cameras.txt -o run/jitter/run --roll-weight 0.1 --yaw-weight 0.1 --heights-from-dem ../data/sat_sim_DEM.tif --heights-from-dem-uncertainty 10 --heights-from-dem-robust-threshold 0.1 --isis-cnet run/run.net --num-anchor-points 500 --anchor-weight 0.001 --anchor-dem ../data/sat_sim_DEM.tif  --camera-position-uncertainty run/camera_uncertainty.txt
