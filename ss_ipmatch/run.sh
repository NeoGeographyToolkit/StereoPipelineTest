...
for method in obalog sift orb; do
    ...
    parse_match_file.py run/${method}/run-left_sub16__right_sub16.match run/${method}/matches.txt
    parse_match_file.py -rev run/${method}/matches.txt run/${method}/run-left_sub16__right_sub16_v2.match

    # Ensure that this tool can read the match file it just created
    parse_match_file.py run/${method}/run-left_sub16__right_sub16_v2.match run/${method}/matches_v2.txt
