...
bundle_adjust                                       \
  ...
  --num-iterations 10                               \
  --image-list                                      \
    run/sensor1_images.txt,run/sensor2_images.txt   \
  --camera-list                                     \
    run/sensor1_cameras.txt,run/sensor2_cameras.txt \
  ../data/gcp_sub16.gcp                             \
  --datum WGS84                                     \
  -o run/ba1/run

# Then just apply a transform
bundle_adjust --matches-as-txt ../data/left_sub16.tif ../data/right_sub16.tif  run/ba1/run-left_sub16.v2.tsai run/ba1/run-right_sub16.v2.tsai --inline-adjustments -t pinhole --datum WGS84 -o run/run --initial-transform run/transform.txt --apply-initial-transform-only --threads 1
