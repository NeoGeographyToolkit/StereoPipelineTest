...
# Create match files. Constrain the camera positions, as otherwise they are
# weakly observable and can drift far along a null space.
bundle_adjust run/f.tif run/n.tif run/f.json run/n.json -o run/run --threads 1 --num-iterations 5 --num-passes 1 --camera-position-uncertainty 100,100

# Solve for jitter with these cameras with the roll and yaw constraints
jitter_solve run/f.tif run/n.tif run/f.json run/n.json -o run/run --match-files-prefix run/run --heights-from-dem ../data/sat_sim_DEM.tif --heights-from-dem-uncertainty 10 --heights-from-dem-robust-threshold 0.1 --roll-weight 100 --yaw-weight 100 --rotation-weight 0.1 --threads 1 --num-iterations 10 --num-passes 1 --camera-position-uncertainty 100,100
...
jitter_solve run/f.tif run/n.tif run/f.json run/n.json -o run/run-weight --match-files-prefix run/run --heights-from-dem ../data/sat_sim_DEM.tif --heights-from-dem-uncertainty 10 --heights-from-dem-robust-threshold 0.1 --threads 1 --weight-image ../data/sat_sim_weight.tif --reference-terrain ../data/sat_sim_DEM.tif --stereo-prefix-list run/stereo_list.txt --max-num-reference-points 1000 --num-iterations 3 --num-passes 1 --mapproj-dem ../data/sat_sim_DEM.tif --camera-position-uncertainty 100,100
